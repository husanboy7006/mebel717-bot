...
// UI: Update MainButton with Queue/Debounce
let mainButtonTimer = null;
const updateMainButton = () => {
    const { items, total } = getCartTotals();
    
    // 1. Update visual total inside the App UI IMMEDIATELY
    const totalEl = document.getElementById('total-price-display');
    if (totalEl) {
        totalEl.textContent = items > 0 ? 'Jami: ' + formatPrice(total) : '';
    }
    
    // 2. Clear any pending updates to Telegram
    clearTimeout(mainButtonTimer);
    
    // 3. Schedule the update to Telegram (Debounce 150ms)
    // This prevents overwhelming the Telegram bridge during rapid clicks
    mainButtonTimer = setTimeout(() => {
        const current = getCartTotals(); // Get fresh state
        if (current.items > 0) {
            const text = state.isCartView
                ? '✅ Buyurtma berish (' + formatPrice(current.total) + ')'
                : '🛒 Savat (' + formatPrice(current.total) + ')';
            ...
            // Rare case: second sync after 50ms just to be absolutely sure
            setTimeout(() => tg.MainButton.setParams(params), 50);
        } else {
            tg.MainButton.hide();
